      ...
      <div class="p-3 border-b border-gray-700 bg-gray-750">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm text-gray-300">Notificaciones Push</span>
          </div>
          <button
            v-if="pushNotifications.isSupported"
            @click="togglePushNotifications"
            :disabled="pushNotifications.loading"
            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors"
            :class="
              pushNotifications.isEnabled ? 'bg-amber-500' : 'bg-gray-600'
            "
          >
            <span
              class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform"
              :class="
                pushNotifications.isEnabled ? 'translate-x-6' : 'translate-x-1'
              "
            />
          </button>
          <span v-else class="text-xs text-gray-500">No soportado</span>
        </div>
      </div>
